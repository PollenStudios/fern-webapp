...
export const Button = ({ variant, name, type }: ButtonProps) => {
  return (
    <button
      type={type}
      className={`heading-6 w-full md:w-auto px-6 py-3 border text-base font-medium rounded-full shadow-sm text-white focus:outline-none 
...
export const ButtonWithIcon = ({ variant, name, type, icon }: ButtonProps) => {
  return (
    <button
      type={type}
      className={`flex justify-center items-center w-full md:w-auto gap-x-4 pl-3 pr-6 py-3 border border-transparent text-base font-medium rounded-full shadow-sm text-white focus:outline-none
      ${checkVariant(variant)}  `}
    >
