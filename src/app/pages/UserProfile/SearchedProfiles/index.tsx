...
function Profiles({query}: any) {
  ...
  const {data, loading, fetchMore} = useSearchProfilesQuery({
    variables: {request},
    skip: !query,
  })
  ...
  if (profiles?.length === 0) {
    return (
      <div className='w-full h-[60vh] flex justify-center items-center heading-4'>
        No Profile found
      </div>
    )
  }
            ...
            <Link to={PageRoutes.USER_PROFILE.split(':')[0] + profile?.id}>
              <div className='rounded-xl border flex hover:border hover:border-primary hover:shadow-sm'>
                ...
                <div className='flex flex-col justify-center ml-2'>
                  ...
                  <h5 className='paragraph-3 sm:paragraph-2  w-32 sm:w-44 truncate text-primary'>
                    {profile.handle}
                  </h5>
                </div>
              </div>
