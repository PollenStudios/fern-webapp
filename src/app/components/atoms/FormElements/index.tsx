...
const tailwindCssClass = {
  inputClass:
    'paragraph-3 rounded-sm border-gray-20  border focus:border-gray-500 focus:ring-gray-100',
}
  ...
  return (
    <div className='relative'>
      ...
      {isDropdownOpen ? (
        <DropDown
          list={options}
          addItem={addItemToMultiSelect}
          selectedItems={selectedItems}
        ></DropDown>
      ) : null}
      {multiSelectError && (
        <p className='paragraph-3 text-red-600 pt-2'>Please select any option</p>
      )}
    </div>
  )
}
...
export default function Select({selected, setSelected, options, setSortPosts}: Iprops) {
  return (
    <Listbox
      ...
      onChange={(art) => {
        setSelected(art)
        art !== ART_CATEGORY_TITLE ? setSortPosts(false) : setSortPosts(true)
      }}
    >
      {({open}) => (
        <>
